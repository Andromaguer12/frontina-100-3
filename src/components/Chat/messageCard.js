...
const MessageCard = ({timestamp, sender, message, id, reference, img}) => {
        ...
        if(sender){
            return (
                <React.Fragment>
                    ...
                    <div className="messagecard" onClick={handleRightClick} style={{ 
                        ...
                        alignSelf: `${sender == user[1]?.id ? "flex-end" : "flex-start"}`,
                        ...
                        <Typography className="msgmetadata" style={{ color: `${sender == user[1]?.id ? "#fff" : "#7a7a7a"}`, padding: `${img ? "2.5px 10px" : "0"}`, justifyContent: `${sender == user[1]?.id ? "flex-end" : "flex-start"}`}}>
                            {getDateFromTimestamp(timestamp).hour}
                        </Typography>
                    </div>
                </React.Fragment>
            ) 
            ...
            return (
                <div className="alertmessagecard"> 
                    <Typography className="alertmessage">{message} at: {getDateFromTimestamp(timestamp).date} | {getDateFromTimestamp(timestamp).hour}</Typography>
                </div>
            )
        }
        return <p>Nothing</p>
