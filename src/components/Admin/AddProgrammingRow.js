...
export default function AddProgrammingRow({cancel}) {
    ...
    const handleNewRow = async (e) => {
        ...
        const array = [form.get("programName"), form.get("synopsis"), form.get("conductor"), form.get("streamDay"), form.get("timeDuration")];
        const verification = []
        array.forEach((input) => {
            if(input === ""){
        ...
        <div className="addShadow">
            <div className="divAddRow">
                ...
                <form style={{ width: "100%" }} onSubmit={handleNewRow}>
                    ...
                    <TextField name="streamDay" style={{ margin: "10px 0" }} fullWidth color="secondary"  variant="outlined" label="Dia de transmision" size="small" />
